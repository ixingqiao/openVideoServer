...
string System::get_sys_local_ip() {
    GET_CONFIG(string, extern_ip, General::kExternIP);
    string local_host = extern_ip;
    do {
        if (!local_host.empty()) {
            break;
        }
        GET_CONFIG(string, eth_name, General::kEthName);
        WarnL << "eth_name:" << eth_name;
        local_host = !eth_name.empty() ? SockUtil::get_ifr_ip(eth_name.c_str()) : local_host;
        if (!local_host.empty()) {
            break;
        }
        local_host = SockUtil::get_local_ip();
    } while (false);
    InfoL << "local_host:" << local_host;
    return local_host;
}
