...
void NtpStamp::update(uint32_t rtp_stamp, double ntp_stamp_ms) {
    if (ntp_stamp_ms == 0) {
    ...
    //与本地系统时间同步
    uint64_t current = getCurrentMillisecond(true);
    uint32_t allowDifference = 5000;
    uint64_t diff = 0;
    //允许最大差值5秒,差值大于5秒则调整ntp时间 
    if (current > (uint64_t)_last_ntp_stamp_ms) {
        diff = current - (uint64_t)_last_ntp_stamp_ms;
    } else {
        diff = (uint64_t)_last_ntp_stamp_ms - current;
    }
    if (diff > allowDifference) {
        //PrintW( "adjust _last_ntp_stamp_ms:%.1f, current:%llu, diff:%llu", _last_ntp_stamp_ms / 1000, current / 1000,diff / 1000);
        _last_ntp_stamp_ms = (double)current;
    }
}
...
uint64_t NtpStamp::getNtpStamp_l(uint32_t rtp_stamp, uint32_t sample_rate) {
    ...
    //rtp时间戳正增长
    if (rtp_stamp >= _last_rtp_stamp) {
        auto diff = static_cast<double>((rtp_stamp - _last_rtp_stamp) * 1.0 / (sample_rate / 1000.0f));
        if (diff < MAX_DELTA_STAMP) {
            //时间戳正常增长
            update(rtp_stamp, _last_ntp_stamp_ms + diff);
            return uint64_t(_last_ntp_stamp_ms);
        }
        PrintW("stamp diff large, rtp_stamp:%lu, last_rtp_stamp:%lu", rtp_stamp, _last_rtp_stamp);
        ...
        if (_last_rtp_stamp < loop_delta && rtp_stamp > UINT32_MAX - loop_delta) {
            ...
            return uint64_t(_last_ntp_stamp_ms + diff - max_rtp_ms);
        }
    ...
    //rtp时间戳负增长
    auto diff = static_cast<double>((_last_rtp_stamp - rtp_stamp) * 1.0 / (sample_rate / 1000.0f));
    if (diff < MAX_DELTA_STAMP) {
        //正常范围的时间戳回退，说明收到rtp乱序了
        return (uint64_t)(_last_ntp_stamp_ms - diff);
    }
